...
/*------------------------ End select page -----------------------------------*/
 ...
 /*======================= View  course Option ======================*/
  	...
  	var AddCoursesDb = dbRef.ref("website/course/AddCourses");
  ...
  AddCoursesRootRef.on("child_added",snap => {
    ...
    var DowloadCoursesDb = dbRef.ref("website/course/AddCourses");
    var DowloadCoursesRootRef = DowloadCoursesDb;

    DowloadCoursesRootRef.on("child_added",snap => {
      var snapkey = snap.key;

      $('#AddCoursesDownload').append("<option id='"+snap.key+"' class='"+'txtType'+"'>"+ snap.key +"</option>");
      $('#AddCoursesDownload').val("");
 ...
 $('#SelectAddCourses').on('click',function(){



 ViewNameCoures = $("#SelectAddCourses").val();
 $('#Suggestion_work').empty();
 var a=0;
 var ViewCoursesDb = dbRef.ref("website/course/AddCourses").child(ViewNameCoures);
 ...
 function editExpert(){
   ...
   uplodadTask.on('state_changed',function(sanpshot){
     ...
     $('#Suggestion_work').empty();
     var deleteRef;
     var deleteImageProfile = firebase.database().ref("website/course/AddCourses").child(editNameCoures).child(idBody).chid('file');
     ...
     firebase.database().ref("website/course/AddCourses").child(editNameCoures).update(updatebachelor);
    ...
    function uploadSuggestion(){
      ...
      if(filesurename == "jpg" || filesurename == "JPG" || filesurename == "png" || filesurename == "PNG"){
        ...
        uplodadTask.on('state_changed',function(sanpshot){
          ...
          TopicNameCoures = $('#SelectAddCourses2').val(),
